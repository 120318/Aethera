...
from app.services.domain.transfer.execution import TransferExecutionContext, build_transfer_execution_context, build_transfer_plan, execute_transfer, generate_source_path, may_inherit_context_episodes, missing_transfer_source_paths, with_context_resource_attrs
...
def test_multi_video_files_do_not_inherit_package_episode_range(monkeypatch):
    ...
    assert with_context_resource_attrs(
        task,
...
def test_selected_single_video_inherits_context_episode_when_torrent_has_unselected_sample(monkeypatch):
    task = _task(status=TaskStatus.FINISHED)
    task.context.selected_files = [0]
    task.context.parsed_attributes = ResourceAttributes(seasons=[1], episodes=[11])
    task.metadata = TorrentMetadata(
        hash="hash-1",
        name="Show.S01E11",
        size=110,
        files=[
            TorrentFileItem(index=0, filename="Show/main.mkv", size=100, attrs=ResourceAttributes(episodes=[])),
            TorrentFileItem(index=1, filename="Show/sample.mkv", size=10, attrs=ResourceAttributes(episodes=[])),
        ],
    )
    context = TransferExecutionContext(
        source_base_path=Path("/downloads"),
    ...
    assert len(results) == 1
    assert results[0].episode_number == 11
    assert results[0].destination_path == "/library/Show (2024)/Season 01/Show - S01E11.mkv"
